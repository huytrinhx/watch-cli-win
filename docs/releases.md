...
Auto-generated from every commit's subject line since the previous tag
(not just PR-merge commits — a tag cut from commits pushed straight to
`main`, no PR, has none of those, and an earlier version of this
generator found nothing and silently produced an empty section), then
human-edited before publish. The `release: vX.Y.Z` bump commit itself
is filtered out — it marks this same release, not a change to list
under it.
...
## What's new
<auto-generated list of commit subject lines since the previous tag,
one per bullet — PR-merge commits get their PR number linked>
